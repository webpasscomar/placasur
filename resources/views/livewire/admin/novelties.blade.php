                    ...
                    <tbody>
                        @foreach ($novelties as $key => $novelty)
                            <tr>
                                ...
                                <td class="align-middle" style="cursor: pointer"
                                    wire:click="openModalImage({{ $novelty->id }})"><img
                                        src="{{ file_exists(public_path('storage/novedades/' . $novelty->image)) ? asset('storage/novedades/' . $novelty->image) : asset('img/no_disponible.png') }}"
                                        alt="{{ $novelty->title }}" width="40" height="40" />
                                </td>
                                <td class="align-middle">{{ $novelty->title }}</td>
                                <td class="align-middle">{{ $novelty->category->name }}</td>
                            ...
                            @if ($showModalImage)
                                ...
                                <x-modal-image
                                    image="{{ file_exists(public_path('storage/novedades/' . $currentImage)) ? asset('storage/novedades/' . $currentImage) : asset('img/no_disponible.png') }}"
