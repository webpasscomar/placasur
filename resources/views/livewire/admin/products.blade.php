                ...
                <table class="table table-hover table-bordered mt-3 datatable" id="myTable">
                    ...
                    @foreach ($rows as $key => $row)
                        <tr>
                            ...
                            <td class="align-middle" style="cursor: pointer"
                                wire:click="openModalImage({{ $row->id }})"><img
                                    src="{{ (file_exists(public_path('storage/productos/' . $row->image)) && $row->image) ? asset('storage/productos/' . $row->image) : asset('img/no_disponible.png') }}"
                        ...
                        @if ($showModalImage)
                            ...
                            <x-modal-image
                                image="{{ (file_exists(public_path('storage/productos/' . $currentImage)) && $currentImage ) ? asset('storage/productos/' . $currentImage) : asset('img/no_disponible.png') }}"
