          ...
          <ul class="nav nav-pills justify-content-center" id="pills-tab" role="tablist">
            @foreach ($categories as $category)
              <li class="nav-item" role="presentation">
                <button class="btn btn-outline-primary rounded-top rounded-0 shadow-sm {{ $loop->first ? 'active' : '' }}"
                  id="{{ strtolower($category->name) }}-tab" data-bs-toggle="tab"
                  data-bs-target="#{{ strtolower($category->name) }}-tab-pane" type="button" role="tab"
                  aria-controls="{{ strtolower($category->name) }}-tab-pane" aria-selected="true">{{ $category->name }}
                </button>
              </li>
            @endforeach
          </ul>
                    ...
                    <div class="row py-3 px-2 shadow rounded-2">
                      <div class="col-md-5 w-50">
                        <img src="{{ asset('/storage/novedades/' . $novelty->image) }}"
                          class="d-block img-fluid rounded-2 object-contain" alt="{{ $novelty->title }}">
                      </div>
                      <div class="col">
                        <span
                        ...
                        <x-modal-novelty id="exampleModal{{ $novelty->id }}" title="{{ $novelty->title }}"
                          date="{{ $novelty->created_at }}" description="{!! $novelty->description !!}"
                          image="{{ $novelty->image }}" />
                      </div>
                    </div>
                  @endif
                      ...
                      @foreach ($categoryNovelty as $novelty)
                        @if ($category->id === $novelty->category_id)
                          <div class="card text-bg-light p-0 info-box__shadow h-100">
                            <div>
                              <img src="{{ asset('/storage/novedades/' . $novelty->image) }}" class="card-img-top"
                                alt="{{ $novelty->title }}">
                            </div>
                            <div class="card-body">
                              <div class="badge rounded-pill border border-primary text-primary mb-3 shadow-sm">
